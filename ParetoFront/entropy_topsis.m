function [ weights,bestvalue,best333index, relative_closeness] = entropy_topsis(pareto_solutions)
    % 输入：pareto_solutions - 多目标遗传算法得到的Pareto解矩阵，每一行代表一个解，
    % 后三列分别为效益型目标和两个成本型目标的值

    % 步骤1: 数据标准化
    % 提取目标值
    objective_values = pareto_solutions(:, end - 1:end);
    ...
    standardized_values(:, 1) = (objective_values(:, 1) - min(objective_values(:, 1))) / ...
       ...
       standardized_values(:, 2) = (objective_values(:, 2) - min(objective_values(:, 2))) / ...
